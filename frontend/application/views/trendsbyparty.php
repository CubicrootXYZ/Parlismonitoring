...
<?php $i = 0;?>
<?php foreach ($values['colors'] as $author => $color): ?>
// Create series
var series<?php echo $i;?> = chart.series.push(new am4charts.LineSeries());
series<?php echo $i;?>.dataFields.valueY = "<?php echo $author;?>";
series<?php echo $i;?>.dataFields.dateX = "date";
series<?php echo $i;?>.tooltipText = "<?php echo $author;?>: {<?php echo $author;?>}";
series<?php echo $i;?>.tensionX = 1;
series<?php echo $i;?>.fillOpacity = 0.2;
series<?php echo $i;?>.strokeWidth = 3;
series<?php echo $i;?>.stroke = am4core.color("<?php echo $color;?>");
series<?php echo $i;?>.fill = am4core.color("<?php echo $color;?>");
...
<div class="row">
  ...
  <small>1 Stichwort eingeben</small>
  <form method="GET">
    <input type="text" name="search" placeholder="Suchwort" value="<?php if (isset($values['search'])) {echo $values['search'];}?>">
...
<br>
  ...
  <?php $i = 0; ?>
  <?php if(isset($values['colors']) ):?>
  <?php foreach ($values['colors'] as $word => $color): ?>
  <span class="badge badge-pill badge-secondary" style="background-color: <?php echo $color;?>"><?php echo $word;?></span>
  <?php $i += 1;
  endforeach;?>
  <?php endif;?>
...
<?php if (strlen($values['data']) < 3 || !isset($values['colors'])): ?>
<div class="container-fluid">
